...
export const publishExam = async (req, res) => {
  try {
    ...
    // Emit Socket.IO event to notify students immediately
    if (exam.courseId && exam.courseId.students && exam.courseId.students.length > 0) {
      try {
        const socketManager = getSocketManager();
        if (socketManager) {
          // Notify each enrolled student via Socket.IO
          for (const studentId of exam.courseId.students) {
            socketManager.emitToUser(studentId.toString(), "exam:published", {
              examId: exam._id,

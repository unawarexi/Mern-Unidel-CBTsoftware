...
// Redis for exam progress caching
import {
  cacheExamProgress,
...
import { getSocketManager } from "../services/socketIO.service.js";
...
export const startExam = async (req, res) => {
  try {
    ...
    // Cache exam progress in Redis
    await cacheExamProgress(submission._id.toString(), {
      examId,
    ...
    // Emit Socket.IO event for exam start
    try {
      const socketManager = getSocketManager();
      if (socketManager) {
        socketManager.emitToUser(studentId.toString(), "exam:started", {
...
export const saveAnswer = async (req, res) => {
  try {
    ...
    // Cache updated progress in Redis
    await cacheExamProgress(id, {
    ...
    // Emit Socket.IO event for answer saved
    try {
      const socketManager = getSocketManager();
      if (socketManager) {
        socketManager.emitToUser(studentId.toString(), "exam:answerSaved", {
          submissionId: id,
          questionId,
...
export const submitExam = async (req, res) => {
  try {
    ...
    // Invalidate exam cache in Redis
    await invalidateExamCache(exam._id.toString());

    // Emit Socket.IO events for exam submission
    try {
      const socketManager = getSocketManager();
      if (socketManager) {
        // Notify student
        socketManager.emitToUser(studentId.toString(), "exam:submitted", {
          submissionId: submission._id,
          examId: exam._id,
          score: submission.score,
        ...
        // Notify lecturer of new submission
        if (exam.lecturerId) {
          socketManager.emitToUser(exam.lecturerId.toString(), "exam:newSubmission", {
            examId: exam._id,
            studentId: studentId.toString(),
            submissionId: submission._id,
            submittedAt: submission.submittedAt,

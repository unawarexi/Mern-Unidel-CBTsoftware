...
// Socket.IO for real-time violation alerts
...
export const reportViolation = async (req, res) => {
  try {
    ...
    const submission = await ExamSubmission.findOne({
      ...
      examId,
      status: "started",
    }).populate("examId", "courseId lecturerId");
    ...
    // Emit Socket.IO event to notify lecturer in real-time
    try {
      const socketManager = getSocketManager();
      if (socketManager && submission.examId.lecturerId) {
        const student = await Student.findById(studentId).select("fullname matricNumber");
        socketManager.emitToUser(submission.examId.lecturerId.toString(), "security:violation", {
          examId,
          submissionId,

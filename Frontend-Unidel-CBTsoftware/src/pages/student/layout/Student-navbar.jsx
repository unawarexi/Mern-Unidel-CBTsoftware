            ...
            <div className="relative">
              <button onClick={() => setProfileDropdownOpen(!profileDropdownOpen)} className="flex items-center space-x-2 sm:space-x-3 p-0.5 sm:p-1 pr-2 sm:pr-3 rounded-lg hover:bg-gray-50 transition-all">
                ...
                <div className="hidden lg:block text-left">
                  <p className="text-xs sm:text-sm font-semibold text-slate-800">{user?.fullname || "Student"}</p>
                  <p className="text-[10px] sm:text-xs text-gray-500">{user?.role ? user.role.charAt(0).toUpperCase() + user.role.slice(1) : "Student"}</p>
                </div>

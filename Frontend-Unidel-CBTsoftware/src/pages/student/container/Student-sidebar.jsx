...
const Sidebar = () => {
  ...
  return (
    <>
      {/* Mobile Menu Button */}
      <button onClick={() => setIsMobileOpen(!isMobileOpen)} className="lg:hidden fixed top-3 sm:top-4 left-3 sm:left-4 z-50 p-2 sm:p-2.5 bg-gradient-to-br from-gray-900 to-gray-800 text-white rounded-xl shadow-lg hover:shadow-xl transition-all">
      ...
      >
        ...
        <div className="h-20 sm:h-28 border-b border-gray-200 flex items-center justify-between px-3 sm:px-4 bg-gradient-to-r from-orange-50 to-orange-100/50">
          <motion.div animate={isCollapsed ? "collapsed" : "expanded"} variants={contentVariants} className="flex items-center gap-2 sm:gap-3 ">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-orange-500 via-orange-600 to-orange-700 rounded-xl flex items-center justify-center shadow-lg">
              <GraduationCap className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
            </div>
            <div>
              <h1 className="font-bold text-gray-900 text-base sm:text-lg">UNIDEL</h1>
              <p className="text-[10px] sm:text-xs font-medium text-orange-600">Student Portal</p>
            </div>
          </motion.div>
        ...
        {/* Main Menu */}
        <nav className="flex-1 overflow-y-auto py-3 sm:py-4 px-2 sm:px-3 space-y-1 sm:space-y-1.5">
          {menuItems.map((item) => (
            <div key={item.id}>
              <button
                onClick={() => (item.subItems ? toggleMenu(item.id) : navigate(item.href))}
                className={`w-full flex items-center justify-between px-2.5 sm:px-3 py-2.5 sm:py-3 rounded-xl transition-all group relative overflow-hidden
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1 relative z-10">
                  <div className={`p-1 sm:p-1.5 rounded-lg ${expandedMenus[item.id] ? "bg-orange-200/50" : "bg-gray-100 group-hover:bg-gray-200"} transition-colors`}>
                    <item.icon className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${expandedMenus[item.id] ? "text-orange-600" : "text-gray-500 group-hover:text-gray-700"}`} />
                  </div>
                  <motion.span animate={isCollapsed ? "collapsed" : "expanded"} variants={contentVariants} className="font-semibold text-xs sm:text-sm truncate">
                    {item.title}
                  </motion.span>
                  {item.badge && (
                    <motion.span animate={isCollapsed ? "collapsed" : "expanded"} variants={contentVariants} className={`px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-xs font-bold rounded-full ${typeof item.badge === "number" ? "bg-orange-500 text-white" : "bg-green-100 text-green-700"}`}>
                      {item.badge}
                    </motion.span>
                  )}
                </div>
              ...
              <AnimatePresence>
                {item.subItems && expandedMenus[item.id] && !isCollapsed && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                    <div className="ml-6 sm:ml-8 mt-1 sm:mt-1.5 space-y-0.5 border-l-2 border-orange-200 pl-2 sm:pl-3">
                      {item.subItems.map((subItem, index) => (
                        <Link key={index} to={subItem.href} className="flex items-center gap-2 px-2.5 sm:px-3 py-2 sm:py-2.5 text-xs sm:text-sm text-gray-600 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-all group">
        ...
        <div className="border-t border-gray-200 p-2 sm:p-3 space-y-1 sm:space-y-1.5 bg-gradient-to-b from-gray-50 to-gray-100">
            ...
            <div key={item.id}>
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className={`p-1 sm:p-1.5 rounded-lg ${item.id === "logout" ? "bg-red-50" : "bg-gray-200"} transition-colors`}>
                    <item.icon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  </div>
                  <motion.span animate={isCollapsed ? "collapsed" : "expanded"} variants={contentVariants} className="font-semibold text-xs sm:text-sm">
                    {item.title}
                  </motion.span>
                </div>
                {item.subItems && (
              ...
              <AnimatePresence>
                {item.subItems && expandedMenus[item.id] && !isCollapsed && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                    <div className="ml-6 sm:ml-8 mt-1 sm:mt-1.5 space-y-0.5 border-l-2 border-gray-300 pl-2 sm:pl-3">
                      {item.subItems.map((subItem, index) => (
                        <Link key={index} to={subItem.href} className="block px-2.5 sm:px-3 py-2 sm:py-2.5 text-xs sm:text-sm text-gray-600 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-all">
                          {subItem.title}
                        </Link>
                      ))}
                    </div>

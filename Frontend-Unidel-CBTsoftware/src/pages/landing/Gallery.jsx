...
import { X, ZoomIn, ArrowRight } from "../../constants/icons";
import { Images } from "../../constants/image-strings";
import useThemeStore from "../../store/theme-store";
import { galleryImages as galleryData } from "../../core/data/landing-mock-data";
import { headerAnimation, galleryImageAnimation, galleryImageHover, ctaAnimation } from "../../core/animation/animations";

const Gallery = () => {
  const [selectedImage, setSelectedImage] = useState(null);
  const { isDarkMode } = useThemeStore();

  // Map image source keys to actual image URLs
  ...
  return (
    <section className={`relative ${isDarkMode ? 'bg-gradient-to-bl from-dark-900/95 via-accent-900/90 to-dark-900/95' : 'bg-gray-50'} py-8 md:py-16 overflow-hidden transition-colors duration-300`}>
      {/* Decorative Background Elements */}
      <div className={`absolute top-20 right-20 w-[400px] h-[400px] ${isDarkMode ? 'bg-primary-600/30' : 'bg-primary-600'} rounded-full blur-3xl animate-glow-pulse`}></div>
      <div className={`absolute bottom-20 left-20 w-[450px] h-[450px] ${isDarkMode ? 'bg-accent-500/30' : 'bg-accent-900'} rounded-full blur-3xl animate-glow-pulse-delayed`}></div>
      ...
      <div className="max-w-[90%] md:max-w-[80%] mx-auto px-3 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-10">
          {galleryImages.map((image, index) => (
            <motion.div
              key={index}
              {...galleryImageAnimation(index)}
              {...galleryImageHover}
              className={`relative group cursor-pointer rounded-2xl overflow-hidden border-2 ${isDarkMode ? 'border-dark-700 hover:border-primary-500/50' : 'border-gray-200 hover:border-primary-300'} transition-all shadow-md hover:shadow-xl`}
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="absolute bottom-0 left-0 right-0 p-3 md:p-5 lg:p-6 text-white">
                  <div className="text-xs font-semibold text-primary-300 mb-1">{image.category}</div>
        ...
        <motion.div
          {...ctaAnimation}
          className={`flex flex-col md:flex-row items-center justify-between ${isDarkMode ? 'bg-dark-800/50 border-dark-700' : 'bg-white border-gray-200'} rounded-2xl border-2 p-4 md:p-8 gap-4 md:gap-6`}
        >

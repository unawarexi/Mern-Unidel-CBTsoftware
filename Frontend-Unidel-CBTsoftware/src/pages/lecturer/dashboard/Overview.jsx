...
const LecturerOverview = () => {
  ...
  const activeExams =
    dashboardStats?.overview?.activeExams ||
    exams.filter((e) => e.status === "active").length ||
    0;
  const pendingExams =
    dashboardStats?.overview?.pendingExams ||
    exams.filter((e) => e.status === "pending").length ||
    0;
  const completedExams = totalExams - activeExams - pendingExams;

  const totalQuestionBanks =
    dashboardStats?.overview?.totalQuestionBanks || questionBanks.length || 0;
  const approvedQuestionBanks =
    dashboardStats?.overview?.approvedQuestionBanks ||
    questionBanks.filter((q) => q.status === "approved").length ||
    0;

  // Calculate pending approval banks from lecturer's own question banks (not all pending approvals)
  const pendingApprovalBanks =
    questionBanks.filter((q) => q.status === "pending_approval").length || 0;
  const draftQuestionBanks =
    questionBanks.filter((q) => q.status === "draft").length || 0;

  const totalStudents =
    dashboardStats?.overview?.totalStudents || students.length || 0;
  ...
  const passRate =
    submissions.length > 0
      ? ((passedSubmissions / submissions.length) * 100).toFixed(1)
      : 0;
  const failRate =
    submissions.length > 0
      ? ((failedSubmissions / submissions.length) * 100).toFixed(1)
      : 0;
  ...
  const performanceMetrics = [
    ...
    },
    {
      ...
      percentage:
        totalSubmissions > 0
          ? ((pendingGrading / totalSubmissions) * 100).toFixed(1)
          : 0,
      color: "bg-orange-500",
      lightBg: "bg-orange-50",
      textColor: "text-orange-600",
  ...
  const examPerformance = (dashboardStats?.performance?.examPerformance || [])
    .slice(0, 6)
    .map((exam) => ({
  ...
  const questionBankStatus = [
    ...
    {
      name: "Rejected",
      value: questionBanks.filter((q) => q.status === "rejected").length,
      color: "#ef4444",
    },
  ];
    ...
    },
    {
      ...
      value:
        totalQuestionBanks > 0
          ? Math.round((approvedQuestionBanks / totalQuestionBanks) * 100)
          : 0,
    },
  ...
  const studentsPerformance = students.slice(0, 10).map((student) => {
    ...
    const avgScore =
      studentSubmissions.length > 0
        ? studentSubmissions.reduce((acc, s) => acc + (s.percentage || 0), 0) /
  ...
  const studentsWithViolations =
    dashboardStats?.security?.studentsWithViolations || [];
  const violationTypeDistribution =
    dashboardStats?.security?.violationTypeDistribution || [];
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        >
          <div className="flex items-center justify-between flex-wrap gap-2 sm:gap-4">
            <div>
              ...
              >
                Manage your courses, exams, and track student performance
              </p>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <select
                ...
                className={cn(
                  "px-2 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-base border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500",
                  isDarkMode
          ...
          {statsLoading || coursesLoading || examsLoading || banksLoading
            ...
            : quickStats.map((stat, idx) => (
                <motion.div
                  ...
                  className={cn(
                    "rounded-xl p-3 sm:p-5 shadow-sm border transition-all hover:shadow-md cursor-pointer",
                    isDarkMode
                ...
                >
                  <div className="flex items-start justify-between mb-2 sm:mb-4">
                    <div className={`${stat.lightBg} p-2 sm:p-3 rounded-lg`}>
                      <div
                        className={
                          stat.color + " text-white rounded-lg p-1 sm:p-2"
                        }
                      >
                  ...
                  <div className={`text-xs ${stat.textColor} mt-0.5 sm:mt-1`}>
                    {stat.subtitle}
                  </div>
                </motion.div>
              ))}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div
                      className={cn(
                        "p-2 rounded-lg",
                        isDarkMode
                          ? `${metric.textColor.replace("text-", "text-opacity-90 text-")} bg-slate-800`
                          : `${metric.lightBg} ${metric.textColor}`,
                      )}
                    >
                      {metric.icon}
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-4">
          ...
          <motion.div
            ...
            animate={{ opacity: 1 }}
            className={cn(
              "lg:col-span-7 rounded-xl p-3 sm:p-5 shadow-sm border",
              isDarkMode
                ? "bg-slate-900/50 backdrop-blur-md border-slate-800"
                : "bg-white border-gray-100",
            )}
            ...
            {statsLoading ? (
              ...
              <ResponsiveContainer width="100%" height={220}>
                <ComposedChart data={activityTrend}>
                  ...
                  />
                  <Bar
                    dataKey="examsCreated"
                    fill="#2563eb"
                    radius={[8, 8, 0, 0]}
                  />
                  <Bar
                    dataKey="banksCreated"
                    fill="#22c55e"
                    radius={[8, 8, 0, 0]}
                  />
                </ComposedChart>
          ...
          <motion.div
            ...
            animate={{ opacity: 1 }}
            className={cn(
              "lg:col-span-5 rounded-xl p-3 sm:p-5 shadow-sm border",
              isDarkMode
                ? "bg-slate-900/50 backdrop-blur-md border-slate-800"
                : "bg-white border-gray-100",
            )}
        ...
        <motion.div
          ...
          animate={{ opacity: 1 }}
          className={cn(
            "rounded-xl p-3 sm:p-5 shadow-sm border mb-4",
            isDarkMode
              ? "bg-slate-900/50 backdrop-blur-md border-slate-800"
              : "bg-white border-gray-100",
          )}
              ...
              {banksLoading ? (
                ...
                recentBanks.map((bank, idx) => (
                  <div
                    ...
                    onClick={() =>
                      navigate(`/lecturer/question-bank/${bank.id}`)
                    }
                  >
                    <div
                      className={cn(
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg sm:text-2xl font-bold text-white mb-1 sm:mb-2">
                  Exam Security Overview
                </h2>
                <p className="text-xs sm:text-base text-white text-opacity-90">
                  Monitor violations in your courses and exams
                </p>
              </div>
                ...
                {statsLoading ? (
                  ...
                  violationsByExam.map((exam, idx) => (
                    ...
                    >
                      ...
                      >
                        Avg: {exam.avgViolationsPerStudent?.toFixed(1)}{" "}
                        violations/student
                      </div>
                    </div>
                  ))
        ...
        <motion.div
          ...
          animate={{ opacity: 1 }}
          className={cn(
            "rounded-xl p-3 sm:p-6 shadow-lg",
            isDarkMode
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3">
            <button
              onClick={() => navigate("/lecturer/exams/create")}
              className="bg-white bg-opacity-10 hover:bg-opacity-20 text-white p-4 rounded-lg transition-all flex flex-col items-center gap-2 border border-white/10"
            >
